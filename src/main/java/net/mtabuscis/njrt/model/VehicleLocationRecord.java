...
public class VehicleLocationRecord extends org.onebusaway.realtime.api.VehicleLocationRecord{
	...
	public boolean makeRecordFromVehActTrip(VehicleActivity va, TripEntry trip, double distanceAlongTrip) {
		
		//From other parts of OBA, the time values should be in Milliseconds
		
	    setVehicleId(AgencyAndId.convertFromString(va.getOperator()+"_"+va.getVehicleId()));
	    
	    setTimeOfRecord(va.getReceivedAtTimestamp().getTime());
	    setTimeOfLocationUpdate(va.getRecordedAtTimestamp().getTime());
	    setBlockId(trip.getBlock().getId());
	    setTripId(trip.getId());
	    setServiceDate(va.getServiceDate().getTime());

	    setCurrentLocationLat(Double.parseDouble(va.getLatitude()));
	    setCurrentLocationLon(Double.parseDouble(va.getLongitude()));
	    
	    //vehicles will always be in progress from NJB
	    setPhase(EVehiclePhase.IN_PROGRESS);
	    setStatus("blockInf");//always blockInf
	    setRunId(va.getRoute()+"-"+trip.getId().getId());
	    
	    setDistanceAlongBlock(distanceAlongTrip);
